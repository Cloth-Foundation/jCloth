...
public final class Lexer {
    ...
    /**
     * Prints detailed information about each token produced by the given {@link Lexer}.
     * This method uses the {@link Lexer} to iterate through the tokens in the
     * input stream and prints information such as token kind, lexeme, keyword, operator,
     * metadata, span (start and end position), and trivia counts (leading/trailing).
     * Additionally, the provided text is printed after all tokens have been processed.
     *
     * @param lexer The lexer instance used to iterate through and retrieve tokens
     *              from the input stream.
     * @param text The input text that corresponds to the tokens being processed.
     */
    public static void printLexer(Lexer lexer, String text) {
        while (true) {
            ...
            int lead = lt.trivia().leading().size();
            int trail = lt.trivia().trailing().size();
            ...
            System.out.println(
                    t.kind() +
                            ...
                            "\top=" + op +
                            "\tmeta=" + meta +
                            "\t[" + t.span().start().getLine() + ":" + t.span().start().getColumn() +
                            ".." + t.span().end().getLine() + ":" + t.span().end().getColumn() +
                            "]" +
                            "\ttrivia=" + lead + "/" + trail
            );
